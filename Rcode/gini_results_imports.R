...
for (Country in unique_countries){
  ...
  #new2=statadata as in its the same type of data
  #now need to do the same with statadata.
  ...
  for(i in years_country) {
    ...
    #adjmatwithout0s = adjmat[rowSums(adjmat[])>0,] 
  ...
  for(i in names(adjacency_matrices)) {
    scale = colSums(adjacency_matrices[[i]])
    scale[scale == 0] <- 1
    normalised_adjacency_matrices[[as.character(i)]] <- scale(adjacency_matrices[[i]], center = FALSE, scale = scale)
    results[[as.character(i)]]<- rowSums(scale(adjacency_matrices[[i]],center=FALSE, scale=scale))
  }
...
giniimports <- bind_rows(gini_results_list_indegree, .id="Country")
write.csv(giniimports, file="gini_results_imports.csv")
