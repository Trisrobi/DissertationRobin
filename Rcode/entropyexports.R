...
for (Country in unique_countries){
  ...
  for(i in names(adjacency_matrices_entropy)) {
    scale = rowSums(adjacency_matrices_entropy[[i]])
    scale[scale == 0] <- 1
    normalised_adjacency_matrices_entropy[[as.character(i)]] <- scale(adjacency_matrices_entropy[[i]], center = FALSE, scale = scale)
    results_entropy[[as.character(i)]]<- colSums(scale(adjacency_matrices_entropy[[i]],center=FALSE, scale=scale))
  }
...
entropy_rowsexports <- bind_rows(entropy_results_list, .id="Country")
write.csv(entropy_rowsexports, file="entropyforexports.csv")
